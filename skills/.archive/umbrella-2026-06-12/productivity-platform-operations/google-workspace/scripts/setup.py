...
SERVICE_SCOPES = {
    ...
    "email-readonly": ["https://www.googleapis.com/auth/gmail.readonly"],
...
def _scopes_for_services(services: str | None) -> list[str]:
    """Return OAuth scopes for a comma-separated service list."""
    if not services or services.strip().lower() == "all":
        return list(SCOPES)
    selected: list[str] = []
    unknown: list[str] = []
    for raw in services.split(","):
        ...
        if not name:
            continue
        scopes = SERVICE_SCOPES.get(name)
        if not scopes:
            unknown.append(name)
            continue
        for scope in scopes:
    ...
    if unknown:
        ...
        print(f"Valid services: {', '.join(sorted(SERVICE_SCOPES))}")
        sys.exit(1)
    if not selected:
...
def exchange_auth_code(code: str):
    ...
    requested_scopes = list(pending_auth.get("requested_scopes") or SCOPES)
...
def main():
    ...
    parser.add_argument(
        "--services",
        default="all",
        help="Comma-separated services for --auth-url: all, email, email-readonly, calendar, drive, drive-readonly, contacts, sheets, docs",
    )
