...
## MCP and plugin interoperability

Antigravity CLI may not expose direct `agy mcp ...` subcommands. Check before assuming MCP support:
...
When the user wants Antigravity to use MCP tools already configured for Claude Code, try importing Claude plugins/config first:
...
Then run a tiny print-mode smoke test asking whether the imported tools are visible before relying on them for real work. Treat this as a capability probe: do not claim Antigravity has MCP access until the imported plugin/tool list or a smoke test verifies it.

For Google Workspace access, prefer a shared local read-only MCP server (for example `google-ro`) over direct shell access when Antigravity can consume it. If Antigravity cannot import/use the MCP server, fall back to Hermes as the broker: Hermes reads Gmail/Drive and passes only the needed excerpts/files into the Antigravity prompt.

## Local usage / quota visibility

As of verified `agy` 1.0.6 on Fausto's machine, Antigravity CLI does not expose a documented `usage`, `quota`, `limits`, `account`, or `status` subcommand. `agy -p /usage` is treated as a normal agent prompt, not as a reliable quota screen, and can cause the agent to inspect files or run tasks.

Local data lives under `/home/fausto/.gemini/antigravity-cli`. Useful non-secret signals:

- `history.jsonl`: user prompts/history entries.
