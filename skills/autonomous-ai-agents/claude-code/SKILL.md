...
Hermes interacts with Claude Code in two fundamentally different ways. Choose based on the task and the local auth behavior.
...
Print mode runs a one-shot task, returns the result, and exits. No PTY needed. No interactive prompts. This is the cleanest integration path **on setups where `claude -p` is authenticated and verified**.
...
- Local-tool backend integrations that wrap Claude Code CLI, e.g. Graphify semantic extraction via `graphify extract <path> --backend claude-cli` for Markdown/docs/KB corpora without external LLM API keys. See `references/graphify-claude-cli.md`.
- Any task where you don't need multi-turn conversation

**Print mode skips ALL interactive dialogs** — no workspace trust prompt, no permission confirmations. This makes it ideal for automation when auth works.
...
### Auth Diagnostic: Interactive vs Print Mode

If `claude -p` fails with an auth error but you need to know whether Claude Code is actually logged in, do a controlled interactive probe in tmux: launch bare `claude`, capture the screen, optionally accept the workspace trust prompt with Enter, capture again, then kill the tmux session. The workspace trust prompt is not an authentication prompt. If the normal TUI appears after trust while `-p` still fails, diagnose the print-mode/auth path separately (`claude auth status --text`, `claude doctor`, env/config review). See `references/interactive-vs-print-auth.md` for the exact disposable tmux commands and interpretation.
...
**External CLI wrappers:** When the user wants Claude Code to delegate to another installed agent CLI, create a user subagent that wraps the CLI in non-interactive print mode. First verify the external CLI with `command -v`, `--version`/`--help`, and a tiny `-p` smoke test. If it prompts for OAuth/login, report "installed but not authenticated", let the user complete auth, then rerun the same smoke test. See `references/external-cli-subagents.md` for the reusable template and Antigravity (`agy`) example.
...
When the user wants Claude Code to access Google Drive/Gmail/Calendar, prefer MCP over ad-hoc shell wrappers. First inspect existing Claude MCP entries:

```
claude mcp list
```

Claude Code may already include official `claude.ai Google Drive`, `claude.ai Gmail`, and `claude.ai Google Calendar` entries. Treat each service independently: Drive can be connected while Gmail/Calendar still need authentication. Official Claude Google MCP is the simplest path for Claude Code, but it uses Claude's own OAuth connection/scopes rather than Hermes' `~/.hermes/google_token.json`.

If the user wants a shared, controlled interface backed by Hermes' existing Google OAuth token, build or configure a local read-only stdio MCP server such as `google-ro` and add it to Claude:

```
claude mcp add -s user google-ro -- python /path/to/hermes-google-ro-mcp/server.py
```

Recommended `google-ro` tool surface:
- `gmail_search(query, max=10)` and `gmail_get(message_id)` only.
- `drive_search(query, max=10, raw_query=false)`, `drive_get(file_id)`, and `drive_download(file_id, filename)` only.
- Optional read-only additions: `calendar_list`, `docs_get`, `sheets_get`.

Implementation pattern: the MCP server calls Hermes' Google compatibility CLI internally:

```
python ${HERMES_HOME:-$HOME/.hermes}/skills/productivity/google-workspace/scripts/google_api.py ...
```

Guardrails for a local Google MCP server:
- Expose only read-only MCP tools; do not register send/reply/upload/share/delete/edit tools.
- Cap result counts (for example max 25-50).
- Download only into a controlled directory such as `/tmp/google-ro-downloads`; do not accept arbitrary output paths.
- Log accesses to a local audit file such as `~/.hermes/logs/google-ro-mcp.log`.
- Remember that MCP is a structured tool interface, not an OS sandbox: if Claude also has unrestricted Bash, it may still access local files. Use `--allowedTools`/`--disallowedTools` to limit broad shell access for sensitive tasks.

In prompts, refer to MCP tools by capability, and if using allowed-tools restrictions include the specific MCP tool names (for example `mcp__google_ro__gmail_search`, `mcp__google_ro__drive_search`) instead of granting broad Bash.
...
## Asynchronous Dispatcher Pattern

When Hermes is operating as a dispatcher/supervisor and the user asks for a substantial task, prefer asynchronous Claude Code orchestration instead of blocking the foreground turn unnecessarily.

For this user, before starting any task execution, explicitly state the delegation choice in one concise line (for example: `Scelta delega: delego a Claude Code`, `Scelta delega: delego a subagent Hermes`, or `Scelta delega: non delego, eseguo direttamente`). Do not include rationale unless the situation is especially complex.

### Bounded background print-mode task
Use this for tasks that can complete without further dialogue but may take minutes:

```
terminal(
  command="claude -p '...task...' --output-format json --max-turns 20",
...
Controller responsibilities after notification:
1. Read the completed process output.
2. Inspect `git status` and relevant diffs.
3. Run the appropriate verification command(s) for the project.
4. Report what Claude did, what Hermes verified, and any remaining failures/questions.

### Long/iterative tmux task
Use tmux for work that may need follow-up prompts, dialog handling, or stuck-state monitoring. Periodically capture the pane and classify the state:
- actively working: tool/activity lines continue changing;
- waiting for input: prompt visible at bottom;
- stuck: permission/trust dialog, repeated error loop, no output change for a long interval;
- finished: prompt returned and task summary/diff/tests are available.

If a task must survive beyond the current turn/session, pair the tmux/background run with a durable watchdog/cron monitor that only reports when the worker is done, stuck, or needs intervention.
...
## Existing-Repository Onboarding Pattern

When the user names an existing repo as a first Claude Code target, do a short controller-side onboarding pass before delegating implementation:

1. Inspect top-level agent/project context files (`AGENTS.md`, `CLAUDE.md`, `PROJECT.md`, package manifests) and git status.
2. Run baseline verification commands that are cheap and documented (for example `npx tsc --noEmit`, `npm run lint`, or project-specific equivalents) so later failures can be classified as pre-existing vs task-introduced.
3. Run a read-only Claude smoke test in the repo to verify Claude Code can load project context without modifying files:
   ```bash
   claude -p 'Read the project context and summarize the repo in one sentence. Do not modify files.' \
     --max-turns 3 --output-format json --allowedTools 'Read'
   ```
4. For substantial work, follow the repo's own worktree/branch convention if documented; otherwise create a task-specific branch/worktree before handing edits to Claude.
5. Include known baseline failures in the task prompt and final report so Claude/Hermes do not misattribute them to the current task.

### Read-Only Repository Analysis Pattern

When the user asks Claude Code to analyze/audit an existing codebase without changing it, keep the controller in charge of guardrails:

1. Read the repo's agent/project context files first (`AGENTS.md`, `PROJECT.md`, `CLAUDE.md`, package manifests) and capture `git status --short --branch` before delegation.
2. Use Claude print mode with a prompt that explicitly says **read-only**, **do not modify files**, **do not install packages**, **do not start persistent servers**, and **avoid dependency/build directories**.
3. Restrict tools with `--allowedTools` to `Read` plus narrowly-scoped safe checks such as `Bash(git status:*)`, `Bash(find:*)`, `Bash(npx tsc --noEmit)`, or `Bash(npm run lint)` as appropriate for the repo.
4. Ask for a structured report in the user's language: architecture, key flows, strengths, risks, type/build/lint findings, prioritized recommendations, and open questions.
5. If Claude exits with `error_max_turns` but has a `session_id`, resume that same session with a final-report prompt instead of starting over.
6. Save the returned report to a markdown file when useful, then re-run `git status --short --branch` and tell the user whether the working tree changed during the analysis.
